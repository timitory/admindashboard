  ...
  <div class="mt-8">
    <div class="mt-8 px-6 pt-6 relative shadow-lg bg-white lg:relative lg:pb-8">
        <div class="lg:flex lg:justify-between">
            <p class="mt-2 mb-4 font-bold text-lg">This contains information of every user on the health cover policy</p>
...
export default {
  ...
  methods: {
    ...
    search(){
      ...
      axios.get(`${baseURL}/admin/health/users/search?search=${this.searchKeyword}`)
      .then(res =>{
    ...
    changePage(num){
      console.log(num)
      this.$store.commit('startLoading')
      axios.get(`${baseURL}/admin/health/users`, {params :{page : num}})
      .then(res=>{
        console.log(res.data.data)
        this.totalRows = res.data.data.totalRecord
  ...
  mounted(){
    this.$store.commit('startLoading')
    axios.get(`${baseURL}/admin/health/users`)
    .then(res =>{
      console.log(res.data.data)
      this.totalRows = res.data.data.totalRecord
      this.policies = res.data.data.records
