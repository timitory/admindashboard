<template>
  <div class="mt-8 py-4 cardbox">
      <div class="px-4 py-8 card">
          <h4 class="font-bold text-2xl">Wallet</h4>
          <div class="mt-4 flex justify-between items-end">
              <svg fill="#0c4a4d" xmlns="http://www.w3.org/2000/svg" width="42.79" height="39.5" viewBox="0 0 42.79 39.5"><g transform="translate(-3.375 -4.499)"><path class="a" d="M39.994,11.25H9.547a6.177,6.177,0,0,0-6.172,6.172V34.7a6.177,6.177,0,0,0,6.172,6.172H39.994A6.177,6.177,0,0,0,46.165,34.7V17.422A6.177,6.177,0,0,0,39.994,11.25Z" transform="translate(0 3.125)"/><path class="a" d="M33.188,4.6,9.026,9.325C7.174,9.736,4.5,11.6,4.5,13.861a6.481,6.481,0,0,1,5.04-1.954H39.061V9.8a5.907,5.907,0,0,0-1.43-3.868h0A4.865,4.865,0,0,0,33.188,4.6Z" transform="translate(0.521)"/></g>
              </svg>
              <router-link to="/app/dashboard/health" class="block font-bold text-green-500 text-base">View Details</router-link>
          </div>
      </div>
      <div class="px-4 py-8 card">
          <h4 class="font-bold text-2xl">Reports</h4>
          <div class="mt-4 flex justify-between items-end">
              <svg fill="#0c4a4d" xmlns="http://www.w3.org/2000/svg" width="39.5" height="39.5" viewBox="0 0 39.5 39.5"><path class="a" d="M32.435,4.5H16.065L4.5,16.065V32.435L16.065,44H32.435L44,32.435V16.065ZM24.25,35.881A2.853,2.853,0,1,1,27.1,33.028,2.847,2.847,0,0,1,24.25,35.881Zm2.194-9.436H22.056V13.278h4.389Z" transform="translate(-4.5 -4.5)"/>
              </svg>
              <router-link to="/app/dashboard/health/reports" class="block font-bold text-green-500 text-base">View Details</router-link>
          </div>
      </div>
      <div class="px-4 py-8 card">
          <h4 class="font-bold opacity-30 text-2xl">Manage Claims</h4>
          <div class="mt-4 flex justify-between items-end">
              <svg fill="#0c4a4d" class="opacity-30" xmlns="http://www.w3.org/2000/svg" width="37.133" height="44.927" viewBox="0 0 37.133 44.927"><g class="a" transform="translate(-44.412)"><g transform="translate(45.885 1.472)"><path class="b" d="M66.285,16.922l-4.94,4.939a.5.5,0,0,0,.355.858h4.94a.5.5,0,0,0,.5-.5V17.277A.5.5,0,0,0,66.285,16.922Z" transform="translate(-61.196 -16.774)"/></g><g transform="translate(64.423 27.805)"><g transform="translate(0)"><path class="b" d="M281.019,316.87a8.561,8.561,0,1,0,8.561,8.561A8.571,8.571,0,0,0,281.019,316.87Zm-1.319,5.22a1.319,1.319,0,1,1,2.638,0v3.341a1.319,1.319,0,0,1-2.638,0Zm2.632,6.962a1.221,1.221,0,0,1-.019.128,1.185,1.185,0,0,1-.032.125c-.012.041-.027.082-.043.121s-.035.079-.055.117a1.251,1.251,0,0,1-.066.111,1.285,1.285,0,0,1-.077.1,1.124,1.124,0,0,1-.087.1c-.03.031-.062.06-.1.088s-.069.053-.1.076a1.273,1.273,0,0,1-.111.066c-.038.02-.077.039-.117.055a1.16,1.16,0,0,1-.121.043,1.043,1.043,0,0,1-.125.032,1.065,1.065,0,0,1-.128.019,1.274,1.274,0,0,1-.129.007,1.29,1.29,0,0,1-.13-.007,1.341,1.341,0,0,1-.253-.051,1.137,1.137,0,0,1-.121-.043c-.04-.017-.079-.035-.117-.055s-.075-.042-.111-.066-.071-.05-.1-.076a1.306,1.306,0,0,1-.1-.088,1.093,1.093,0,0,1-.087-.1,1.26,1.26,0,0,1-.077-.1,1.1,1.1,0,0,1-.067-.111,1.46,1.46,0,0,1-.1-.238c-.012-.04-.023-.083-.032-.125a1.289,1.289,0,0,1-.019-.128,1.256,1.256,0,0,1-.006-.129c0-.043,0-.087.006-.13a1.259,1.259,0,0,1,.019-.127c.009-.042.019-.084.032-.126a1.254,1.254,0,0,1,.044-.121,1.168,1.168,0,0,1,.054-.117,1.4,1.4,0,0,1,.143-.215,1.281,1.281,0,0,1,.087-.1,1.3,1.3,0,0,1,.1-.087,1.149,1.149,0,0,1,.1-.077c.036-.024.073-.047.111-.067a1.155,1.155,0,0,1,.117-.055,1.17,1.17,0,0,1,.121-.044c.041-.012.083-.023.126-.032s.085-.015.128-.019a1.294,1.294,0,0,1,.259,0,1.258,1.258,0,0,1,.128.019c.042.009.083.019.125.032a1.454,1.454,0,0,1,.238.1c.038.02.076.043.111.067a1.272,1.272,0,0,1,.1.077,1.285,1.285,0,0,1,.183.184,1.146,1.146,0,0,1,.077.1q.036.054.066.111c.02.038.039.077.055.117s.031.081.043.121a1.2,1.2,0,0,1,.032.126,1.25,1.25,0,0,1,.019.127,1.265,1.265,0,0,1,.007.13A1.247,1.247,0,0,1,282.332,329.052Z" transform="translate(-272.458 -316.87)"/></g></g><g transform="translate(44.412)"><g transform="translate(0)"><path class="b" d="M70.584,0H54.972a.5.5,0,0,0-.5.5V8.674c0,.01,0,.02,0,.031s0,.02,0,.031a1.319,1.319,0,0,1-1.319,1.319H44.914a.5.5,0,0,0-.5.5V35.162a3.719,3.719,0,0,0,3.719,3.719H61.994a.916.916,0,0,0,.9-1.089,10.329,10.329,0,0,1-.188-1.969c0-5.677,4.6-9.839,10.279-9.839q.187,0,.373.007a.915.915,0,0,0,.946-.916V3.719A3.719,3.719,0,0,0,70.584,0ZM57.723,31.3H51.011a1.319,1.319,0,0,1,0-2.638h6.712a1.319,1.319,0,0,1,0,2.638ZM67.7,26.052H51.011a1.319,1.319,0,1,1,0-2.638H67.7a1.319,1.319,0,0,1,0,2.638Zm0-5.292H51.011a1.319,1.319,0,1,1,0-2.638H67.7a1.319,1.319,0,0,1,0,2.638Zm0-5.089H51.011a1.319,1.319,0,1,1,0-2.638H67.7a1.319,1.319,0,0,1,0,2.638Z" transform="translate(-44.412)"/></g></g></g>
              </svg>
              <p class="block opacity-30 font-bold text-green-500 text-base">View Details</p>
          </div>
      </div>
      <div class="px-4 py-8 card">
          <h4 class="font-bold text-2xl">Partners</h4>
          <div class="mt-4 flex justify-between items-end">
              <svg fill="#0c4a4d" xmlns="http://www.w3.org/2000/svg" width="63.645" height="37.126" viewBox="0 0 63.645 37.126"><g transform="translate(0 -106.666)"><g transform="translate(0 106.666)"><g transform="translate(0 0)"><path class="a" d="M15.381,109.583c-2.122-2.122-11.138-2.652-14.055-2.917a.974.974,0,0,0-.8.265A2.011,2.011,0,0,0,0,107.992v23.867a1.253,1.253,0,0,0,1.326,1.326H9.282a1.458,1.458,0,0,0,1.326-.8c0-.8,4.773-14.85,5.3-21.745A1.059,1.059,0,0,0,15.381,109.583Z" transform="translate(0 -106.666)"/></g></g><g transform="translate(22.507 112.235)"><g transform="translate(0 0)"><path class="a" d="M202.31,155.179c-2.652-.8-5.038-1.856-7.16-2.652-4.773-2.122-6.895-1.061-11.138,3.182-1.856,1.856-3.182,4.508-2.917,5.569,0,.265,0,.265.53.53,1.326.53,3.182.8,5.038-2.122a1.378,1.378,0,0,1,1.061-.53,4.915,4.915,0,0,0,1.856-.53,12.928,12.928,0,0,1,1.856-.8h.53c.265,0,.8.265,1.061.265,1.326,1.326,3.713,3.182,6.1,5.3,3.713,2.917,7.425,6.1,9.282,8.486h.265C206.818,166.582,203.636,157.566,202.31,155.179Z" transform="translate(-181.061 -151.467)"/></g></g><g transform="translate(45.081 109.318)"><g transform="translate(0 0)"><path class="a" d="M379.9,128a46.5,46.5,0,0,0-16.442,2.652l-.8.8a1.6,1.6,0,0,0,.265,1.061c1.591,2.652,6.895,17.237,7.69,20.95a1.762,1.762,0,0,0,1.326,1.061H379.9a1.253,1.253,0,0,0,1.326-1.326V129.326A1.253,1.253,0,0,0,379.9,128Z" transform="translate(-362.666 -128)"/></g></g><g transform="translate(13.79 114.622)"><g transform="translate(0 0)"><path class="a" d="M145.143,191.351c-1.061-2.387-5.569-5.834-9.547-9.016-2.121-1.856-4.243-3.447-5.834-4.773-.265.265-.8.265-.8.53a5,5,0,0,1-2.122.53c-2.121,2.652-4.773,3.713-7.69,2.652a3.135,3.135,0,0,1-2.387-2.387c-.53-2.652,1.856-6.364,3.713-8.221h-5.3a135.2,135.2,0,0,1-4.243,15.911,29.976,29.976,0,0,0,2.917,2.917c5.038,4.243,10.873,8.486,11.933,9.282a7.538,7.538,0,0,0,3.182,1.061h.8l-4.243-4.243a1.313,1.313,0,0,1,1.856-1.856l5.3,5.3c.53.53,1.061.265,1.591.265.8-.265,1.061-.8,1.326-1.591l-6.1-6.1a1.313,1.313,0,1,1,1.856-1.856l6.63,6.63a2.836,2.836,0,0,0,2.122,0c.265-.265.8-.53,1.061-1.061L133.74,187.9a1.313,1.313,0,0,1,1.856-1.856l7.69,7.69a1.754,1.754,0,0,0,1.591-.265A1.8,1.8,0,0,0,145.143,191.351Z" transform="translate(-110.934 -170.667)"/></g></g></g>
              </svg>
              <router-link to="/app/dashboard/health" class="block font-bold text-green-500 text-base">View Details</router-link>
          </div>
      </div>
      <div class="px-4 py-8 card">
          <h4 class="font-bold text-2xl">Repayments</h4>
          <div class="mt-4 flex justify-between items-end">
              <svg xmlns="http://www.w3.org/2000/svg" width="50.243" height="37.356" viewBox="0 0 50.243 37.356"><g transform="translate(-24 -530)">
                  <path fill="#0c4a4d" class="a" d="M0,28.9a3.2,3.2,0,0,0,3.2,3.2H35.177a3.2,3.2,0,0,0,3.2-3.2V17.174H0Zm12.792-4.53a.8.8,0,0,1,.8-.8h9.061a.8.8,0,0,1,.8.8v2.665a.8.8,0,0,1-.8.8H13.591a.8.8,0,0,1-.8-.8Zm-8.528,0a.8.8,0,0,1,.8-.8h4.8a.8.8,0,0,1,.8.8v2.665a.8.8,0,0,1-.8.8h-4.8a.8.8,0,0,1-.8-.8ZM38.375,5.448v3.2H0v-3.2a3.2,3.2,0,0,1,3.2-3.2H35.177A3.2,3.2,0,0,1,38.375,5.448Z" transform="translate(35.867 535.259)"/><g transform="translate(24 530)"><circle fill="#0c4a4d" class="a" cx="13.931" cy="13.931" r="13.931"/>
                  <path fill="#fff" class="b" d="M11.724,19.62a6.842,6.842,0,0,0,.952,3.514,6.968,6.968,0,0,0,2.562,2.548,6.782,6.782,0,0,0,3.5.939,6.928,6.928,0,0,0,3.527-.925A6.763,6.763,0,0,0,24.8,23.161a6.964,6.964,0,0,0,.925-3.527,1.145,1.145,0,0,0-.322-.818,1.025,1.025,0,0,0-.791-.335,1.145,1.145,0,0,0-1.14,1.153,4.762,4.762,0,0,1-4.761,4.761A4.478,4.478,0,0,1,15.4,23a4.626,4.626,0,0,1-1.368-3.366,4.955,4.955,0,0,1,1.06-3.1,3.334,3.334,0,0,1,2.468-1.475l-.51.5a1.144,1.144,0,0,0-.322.778,1.183,1.183,0,0,0,.322.8,1.11,1.11,0,0,0,1.623,0L21.139,14.7a.98.98,0,0,0,.322-.832.893.893,0,0,0-.322-.764L18.7,10.594a1.149,1.149,0,0,0-1.609.013,1.094,1.094,0,0,0-.335.8,1.123,1.123,0,0,0,.322.818l.51.483A6.9,6.9,0,0,0,13.4,15.127,6.748,6.748,0,0,0,11.724,19.62Z" transform="translate(-4.793 -5.518)"/></g></g>
              </svg>
              <router-link to="/app/dashboard/health/repayments" class="block font-bold text-green-500 text-base">View Details</router-link>
          </div>
      </div>
      <div class="px-4 py-8 card">
          <h4 class="font-bold text-2xl">Managed Failed Transactions</h4>
          <div class="mt-4 flex justify-between items-end">
              <svg xmlns="http://www.w3.org/2000/svg" width="36.272" height="36.272" viewBox="0 0 36.272 36.272">
                <path fill="#0c4a4d" class="a" d="M18.207,0A18.207,18.207,0,0,0,0,18.065,18.329,18.329,0,0,0,18.207,36.272,18.136,18.136,0,0,0,18.207,0Zm8.942,24a2.126,2.126,0,0,1-3.007,3.007L18.207,21.07l-6.078,5.938A2.126,2.126,0,0,1,9.123,24l5.937-5.937L9.123,12.128a2.126,2.126,0,0,1,3.007-3.007l6.078,5.938,5.935-5.938a2.126,2.126,0,1,1,3.007,3.007l-5.937,5.937Z"/>
              </svg>
              <router-link to="/app/dashboard/health/failedtransactions" class="block font-bold text-green-500 text-base">View Details</router-link>
          </div>
      </div>
      <div class="px-4 py-8 card">
          <h4 class="font-bold text-2xl">All Policies</h4>
          <div class="mt-4 flex justify-between items-end">
              <svg xmlns="http://www.w3.org/2000/svg" width="37.65" height="42.5" viewBox="0 0 37.65 42.5"><g transform="translate(-17.126)">
                  <path fill="#0c4a4d" class="a" d="M21.657,37.148a14.316,14.316,0,0,1,8.958-13.26l2.677-5.377a18.826,18.826,0,0,0-2.414,36.766l.515-4.581A14.319,14.319,0,0,1,21.657,37.148Z" transform="translate(0 -15.891)"/>
                  <path fill="#0c4a4d" class="a" d="M185.126,40.229A18.858,18.858,0,0,0,171.38,22.1l-.526,4.578a14.294,14.294,0,0,1,.875,26.773l-2.751,5.411A18.853,18.853,0,0,0,185.126,40.229Z" transform="translate(-130.35 -18.972)"/>
                  <path fill="#0c4a4d" class="a" d="M94.781,18.976a1.047,1.047,0,0,0-1.01-1.542H85.159L87.013,1.317a1.2,1.2,0,0,0-.8-1.269A1.153,1.153,0,0,0,85.882,0a1.13,1.13,0,0,0-1.008.629l-11.647,23.4a1.233,1.233,0,0,0,.051,1.152,1.2,1.2,0,0,0,.963.605h8.5L81,41.3a1.074,1.074,0,0,0,.8,1.159,1.121,1.121,0,0,0,.325.041,1.133,1.133,0,0,0,1.01-.62Z" transform="translate(-48.055 0)"/></g>
              </svg>
              <router-link to="/app/dashboard/health/policies" class="block font-bold text-green-500 text-base">View Details</router-link>
          </div>
      </div>
      <div class="px-4 py-8 card">
          <h4 class="font-bold text-2xl">Customer Data</h4>
          <div class="mt-4 flex justify-between items-end">
              <svg xmlns="http://www.w3.org/2000/svg" width="32.373" height="42.5" viewBox="0 0 32.373 42.5"><g transform="translate(-61)">
                  <path fill="#0c4a4d" class="a" d="M351,8.787v7.571h7.571Z" transform="translate(-265.928 -8.058)"/>
                  <path fill="#0c4a4d" class="a" d="M180.619,192h-5.126A6.569,6.569,0,0,1,171,195.2v5.976a10.335,10.335,0,0,0,7.056,9.85,10.335,10.335,0,0,0,7.056-9.85V195.2A6.57,6.57,0,0,1,180.619,192Zm1.839,8.079-4.7,4.7a1.245,1.245,0,0,1-1.761,0l-2.348-2.348a1.245,1.245,0,0,1,1.761-1.761l1.467,1.467,3.815-3.815a1.245,1.245,0,1,1,1.761,1.761Z" transform="translate(-100.869 -176.062)"/>
                  <path fill="#0c4a4d" class="a" d="M83.827,10.791a1.245,1.245,0,0,1-1.245-1.245V0H65.565A4.571,4.571,0,0,0,61,4.565V37.935A4.571,4.571,0,0,0,65.565,42.5H88.808a4.571,4.571,0,0,0,4.565-4.565V10.791Zm2.905,14.327a12.893,12.893,0,0,1-9.188,12.349,1.244,1.244,0,0,1-.716,0,12.893,12.893,0,0,1-9.188-12.349V18.013a1.245,1.245,0,0,1,1.245-1.245,4.1,4.1,0,0,0,3.692-2.281l.175-.35a1.245,1.245,0,0,1,1.114-.688h6.641a1.245,1.245,0,0,1,1.114.688l.175.351a4.1,4.1,0,0,0,3.692,2.281,1.245,1.245,0,0,1,1.245,1.245Z"/></g>
              </svg>
              <router-link to="/app/dashboard/health/customers" class="block font-bold text-green-500 text-base">View Details</router-link>
          </div>
      </div>
  </div>
</template>

<script>
export default {

}
</script>

<style scoped>
h4{
    color: #0c4a4d
}
.card{
    /* border: 0.20000000298023224px solid #707070; */
    box-shadow: 20px 20px 33px#00000029;
    border: 1px solid #707070;
    border-radius: 10px;
    margin-bottom: 20px;
}
@media only screen and (min-width: 1024px){
    .cardbox{
        display: flex;
        flex-wrap: wrap;
        justify-content: space-between;
    }
    .card{
        flex: 0 0 22%
    }
}
</style>