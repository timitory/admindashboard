          ...
          <tbody>
            <tr v-for="(policy, index) in filteredClaims" :key="index" class="border border-solid border-gray-300">
             ...
             <td> 
                ...
                <p v-else  class="text-sm bg-yellow-500 text-white p-1 rounded text-center">{{policy.status}}</p>
              </td>
              <td>
...
import SettleModal from "@/components/Home/SettleModal"
...
export default {
  ...
  methods: {
    ...
    getClaims(){
      ...
      axios.get(`${baseURL}/admin/claims?type=home`)
      .then((res)=>{
