...
import User from "@/components/Vehicle/SingleUser"
export default {
  components:{
    TPagination, User
  },
  ...
  methods: {
    ...
    search(){
      ...
      axios.get(`${baseURL}/admin/vehicle/users/search?search=${this.searchKeyword}`)
      .then(res =>{
  ...
  mounted(){
    this.$store.commit('startLoading')
    axios.get(`${baseURL}/admin/vehicle/users`)
    .then(res =>{
      console.log(res.data.data)
      this.totalRows = res.data.data.totalRecord
      this.policies = res.data.data.records
