    ...
    <div class="mt-8">
        <div class="mt-8 px-6 pt-6 relative shadow-lg bg-white lg:relative lg:pb-8">
            <div class="lg:flex lg:justify-between toggle-claim-h">
                ...
                <div class="lg:flex lg:gap-4">
                    <download-excel :data="policiess" :name="fileName" class="right">
                        <button type="button" class="flex mt-4 items-center py-2 px-2 rounded text-white"
                            style="background-color: #131B47; max-width: 180px">Download CSV</button>
                    </download-excel>
                    <div class="relative">

                        <input type="text" v-model="searchKeyword" @change="search()"
            ...
            <div class="overflow-x-auto xl:overflow-x-hidden tablecont">
                <table v-if="paginatedClaims.length > 0" class="w-full mt-8">
                    <thead>
                        <tr>
                    ...
                    <tbody>
                        <tr v-for="(policy, index) in paginatedClaims" :key="index"
                            class="border border-solid border-gray-300">
                            <td>{{ policy.user === null ? '' : policy.user.firstname || '' }} {{ policy.user === null ? '' : policy.user.lastname }}</td>
                            <td>{{ policy.user === null ? '' : policy.user.email }}</td>
                            <td>{{ policy.underwriter === null ? '' : policy.underwriter.name }}</td>
                            <!-- <td> -->
                                <!-- {{policy.metadata['start_date']}} -->
                            <!-- </td> -->
                            ...
                            <td>
                                <p v-if="policy.status == 'Settled'"
                                    class="text-sm bg-green-500 text-white p-1 rounded text-center">Settled</p>
                                <p v-else-if="policy.underwriter_status == 'Accept'"
                                    class="text-sm bg-green-500 text-white p-1 rounded text-center">Approved</p>
                                <p v-else-if="policy.underwriter_status == 'Decline'"
                                    class="text-sm bg-red-500 text-white p-1 rounded text-center">Declined</p>
                                <p v-else-if="policy.status == 'Accept'"
                                    class="text-sm bg-green-500 text-white p-1 rounded text-center">Accepted</p>

                                <p v-else-if="policy.status == 'Pending'"
                                    class="text-sm bg-yellow-500 text-white p-1 rounded text-center">{{ policy.status }}</p>
                                <p v-else class="text-sm bg-red-500 text-white p-1 rounded text-center">{{ policy.status }}
                                </p>
                            </td>
                            <td>
                                <select class="border rounded focus:outline-none" v-model="action"
                                    @change="selectAction(policy)">
                                    <option disabled value="">Action</option>
                                    <option
                                        v-if="policy.status != 'Accept' && policy.status != 'Decline' && policy.status != 'Settled'"
                                        value="approve">Accept</option>
                                    <option v-if="policy.status == 'Pending'" value="decline">Decline</option>
                                    <option
                                        v-if="policy.status != 'Decline' && policy.status != 'Settled' && policy.underwriter_status == 'Accept'"
                                        value="settle">Mark as Settled</option>
                                    <option v-if="policy.status != 'Decline' && policy.status == 'Settled'" value="settle">
                                        Edit Settled Amount</option>
                                </select>
                            </td>
                            <!-- <td>
                                <button @click="viewClaim(policy)"
                                    class="text-green-500 text-sm underline outline-none focus:outline-none">View
                                    More</button>
                            </td> -->
                        </tr>
                    </tbody>
                ...
                <div class="my-8">
                    <t-pagination :total-items="totalRecords" :per-page="perPage" :limit="limit" :disabled="disabled"
