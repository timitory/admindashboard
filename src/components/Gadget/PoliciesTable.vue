<template>
  <div class="mt-8">
    ...
    <div class="mt-8 px-6 pt-6 relative shadow-lg bg-white lg:relative lg:pb-8">
        ...
        <button type="button" class="flex mt-4 items-center py-2 px-2 rounded text-white" style="background-color: #131B47; max-width: 180px">Downloads CSV</button>
    </download-excel>
                <div class="relative">
        ...
        <table v-if="paginatedPolicies.length > 0" class="w-full mt-8">
          <thead>
            <tr>
              ...
              <th class="font-bold">Payment Frequency</th>
          ...
          <tbody>
            <tr v-for="(policy, index) in paginatedPolicies" :key="index" class="border border-solid border-gray-300">
              ...
              <td>{{policy.user.telephone}}</td>
              <td>
                <span v-if="policy.status == 'Success'" class="rounded text-white text-center p-1 bg-green-500">{{policy.status}}</span>
                <span v-else-if="policy.status == 'Active'" class="rounded text-white text-center p-1 bg-green-500">{{policy.status}}</span>
                <span v-else-if="policy.status == 'Incomplete'" class="rounded text-white text-center p-1 bg-red-500">{{policy.status}}</span>
                <span v-else-if="policy.status == 'Pending'" class="rounded text-white text-center p-1 bg-yellow-500">{{policy.status}}</span>
                <span v-else class="rounded text-white text-center p-1 bg-red-500">{{policy.status}}</span>
              </td>
...
export default {
  ...
  methods: {
    ...
    search(){
      this.$store.commit('startLoading')
      axios.get(`${baseURL}/admin/pet/policy/search?search=${this.searchKeyword}`)
      .then(res =>{
        console.log(res.data.data)
